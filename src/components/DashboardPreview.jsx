      ...
      <div ref={ref} className="mx-auto max-w-7xl px-6">
        <div className={`text-center mb-16 transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <p className="mt-4 text-white/40 text-lg max-w-xl mx-auto">
            and actionable insights at your fingertips
          </p>
        </div>
        ...
        <div className={`relative transition-all duration-1000 delay-200 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
          ...
          <div className="relative bg-[#0a1628] rounded-3xl p-6 md:p-8 shadow-2xl border border-white/5 overflow-hidden">
                ...
                {topStats.map((stat) => (
                  <div key={stat.label} className="bg-white/[0.03] rounded-xl p-5 border border-white/5 hover:bg-white/[0.06] hover:border-white/10 transition-all duration-300 group">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-[11px] font-medium ${stat.up ? "text-emerald-400" : "text-red-400"}`}>
                        {stat.change}
                      </span>
                  ...
                  <div className="flex items-end gap-2 h-36">
                    {[35, 55, 45, 70, 50, 80, 65, 75, 55, 90, 70, 85].map((h, i) => (
                      <div key={i} className="flex-1 group cursor-default">
                        <div
                          className="w-full rounded-sm bg-gradient-to-t from-[#2563eb] to-[#06b6d4] transition-all duration-300 group-hover:brightness-125 group-hover:shadow-lg group-hover:shadow-blue-500/20 relative"
                          style={{ height: `${h}%` }}
                        >
                  ...
                  <div className="flex flex-col gap-4">
                    {countries.map((c) => (
                      <div key={c.name} className="flex items-center gap-3 group">
                        <span className="text-sm text-white/50 w-32 truncate group-hover:text-white/70 transition-colors">{c.name}</span>
                        <div className="flex-1 h-2 bg-white/[0.04] rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[#2563eb] to-[#06b6d4] rounded-full transition-all duration-500 group-hover:brightness-125"
                            style={{ width: `${c.pct}%` }}
                          />
                        </div>
